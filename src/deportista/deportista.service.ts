...
export class DeportistaService {
  ...
  async remove(id: number): Promise<void> {
    ...
    if (deportista.foto) {
      deletePromises.push(this.cloudinaryService.deleteImage(deportista.foto).catch(() => {}));
    }
    if (deportista.documentoIdentidadPdf) {
      deletePromises.push(this.cloudinaryService.deleteImage(deportista.documentoIdentidadPdf).catch(() => {}));
    }
    if (deportista.registroCivilPdf) {
      deletePromises.push(this.cloudinaryService.deleteImage(deportista.registroCivilPdf).catch(() => {}));
    }
    if (deportista.afiliacionPdf) {
      deletePromises.push(this.cloudinaryService.deleteImage(deportista.afiliacionPdf).catch(() => {}));
    }
    if (deportista.certificadoEpsPdf) {
      deletePromises.push(this.cloudinaryService.deleteImage(deportista.certificadoEpsPdf).catch(() => {}));
    }
    if (deportista.permisoResponsablePdf) {
      deletePromises.push(this.cloudinaryService.deleteImage(deportista.permisoResponsablePdf).catch(() => {}));
    }

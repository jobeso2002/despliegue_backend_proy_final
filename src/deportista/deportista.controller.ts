...
export class DeportistaController {
  ...
  @ApiBearerAuth('mi secreto1')
  @Delete(':id')
  @ApiOperation({ summary: 'Eliminar permanentemente un deportista' })
  @ApiResponse({ status: 200, description: 'Deportista eliminado permanentemente' })
  @ApiResponse({ status: 404, description: 'Deportista no encontrado' })
  async remove(@Param('id') id: string) {
    await this.deportistaService.remove(+id);
    return { message: 'Deportista eliminado permanentemente' };
  }

...
export class ResultadoService {
  ...
  async create(createResultadoDto: CreateResultadoDto): Promise<Resultado> {
    const { partidoId, usuarioRegistraId, sets, ...resultadoData } = createResultadoDto;
...
// Validar que los sets suministrados coinciden con los sets ganados
const setsGanadosLocal = sets.filter(s => s.puntosLocal > s.puntosVisitante).length;
const setsGanadosVisitante = sets.filter(s => s.puntosVisitante > s.puntosLocal).length;

if (setsGanadosLocal !== resultadoData.setsLocal || 
    setsGanadosVisitante !== resultadoData.setsVisitante) {
  throw new BadRequestException('La cantidad de sets ganados no coincide con los puntos por set');
}

// Validar que el quinto set (si existe) es a 15 puntos
const fifthSet = sets.find(s => s.numeroSet === 5);
if (fifthSet && (fifthSet.puntosLocal < 15 && fifthSet.puntosVisitante < 15)) {
  throw new BadRequestException('El quinto set debe ser a 15 puntos');
}
